...
class WavHandler:
    ...
    def Start(self, filePath: str) -> None:
        ...
        try:
            with wave.open(filePath, "r") as wav:
                ...
                # 双声道 / 单声道
                self.pcmData = np.frombuffer(wav.readframes(self.numFrames),
                                             dtype=np.int16 if self.sampleWidth == 2 else np.int32)
                # 标准化
    ...
    def GetDb(self) -> float:
        reference = 2147483647.0 if self.sampleWidth == 4 else 32767.0  # 使用精确的参考值
        if self.currentRms > 0:
            db = 20 * np.log10(self.currentRms / reference)
        else:
            db = -np.inf  # 表示无信号
        return db
    ...
    def Update(self) -> bool:
        ...
        dataFragment = self.pcmData[:, self.lastOffset:currentOffset].astype(np.float64)
